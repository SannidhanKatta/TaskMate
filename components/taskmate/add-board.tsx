...
const AddBoard = ({ setAddBoardModul }: AddBoardProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        router.push(
          `/taskmate/board?board=${newBoard.data.name}&id=${newBoard.data.id}`
        );
        setAddBoardModul(false);
  ...
  return (
    <div className={`${darkMode ? 'bg-[#2B2C37] text-white border-[#3E3F4E]' : 'bg-white'} absolute w-[520px] mx-auto mt-[0%] rounded-xl p-[32px] pb-[48px] h-auto shadow-lg left-[35%] z-40`}>
      <div className="mb-4 text-xl font-bold">Add New Board</div>
      <Form.Root className="w-full" onSubmit={handleSubmit}>
        ...
        <div className="flex flex-row gap-2">
          <Form.Submit asChild>
            <button
              className={`mt-6 flex justify-center text-center w-full h-10 bg-indigo-500 hover:bg-indigo-700 rounded-2xl align-middle items-center disabled:bg-indigo-200 disabled:cursor-not-allowed ${loading ? "animate-pulse cursor-not-allowed" : "cursor-pointer"
                }`}
              disabled={
                name.length === 0 ||
          ...
          <div
            className="flex justify-center items-center mt-6 w-40 h-10 text-center align-middle rounded-2xl cursor-pointer"
            style={{
              transition: "200ms ease-in",
            }}
          >
            ...
            >
              <div className="flex flex-row gap-2 items-center text-xs tracking-widest uppercase align-middle text-slate-500">
                Cancel
              </div>
            </button>
          </div>
