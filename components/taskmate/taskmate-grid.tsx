...
import KanbanCard from "./taskmate-card";
...
import { commonClasses, addColumnContainerClasses } from "./taskmate-grid-styles";
...
const KanbanGrid = ({
  ...
  return (
    <>
      ...
      {/* Main Board Content */}
      <KanbanHeader />
      <div className="board-content h-[calc(100vh-80px)] mt-[80px]">
        <div className="grid overflow-x-auto grid-flow-col auto-cols-max gap-6 px-12 w-full h-full">
          {columns.map((col) => {
            ...
            if (colIndex !== -1 && col.boardId === boardId) {
              return (
                <div
                  key={col.id}
                  className={`overflow-y-auto px-4 py-1 border-2 ${columnClasses} w-[300px] h-fit max-h-[calc(100vh-120px)]`}
                >
